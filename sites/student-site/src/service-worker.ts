...
const CACHE_VERSION = `autonomy-student-v${version}`;
      ...
      caches.keys().then((keys) =>
        Promise.all(
          keys
            .filter(
              (key) =>
                key.startsWith("static-autonomy-student-") ||
                key.startsWith("fonts-autonomy-student-") ||
                key.startsWith("runtime-autonomy-student-"),
            )
