...
import { user, session as sessionTable, document, documentShare, conversionTask } from "$lib/server/db/schema";
  ...
  if (isAdmin) {
    ...
    if (query) {
      searchResults = await db
        ...
        .from(user)
        .where(or(sql`${user.email} LIKE ${"%" + query + "%"}`, sql`${user.name} LIKE ${"%" + query + "%"}`))
        .limit(10);
    }
  }
...
export const actions: Actions = {
  uploadAndConvert: async ({ request, locals }) => {
    ...
    try {
      ...
      return { success: true, message: "Document converted successfully.", markdown: convertedMarkdown };
    } catch (err: any) {
      await db
        .update(conversionTask)
        .set({
