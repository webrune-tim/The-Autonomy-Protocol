...
const staggerText: Action<HTMLElement, StaggerOptions | undefined> = (
  ...
  const { delay = 0.05, duration = 0.3, intervalSpeed = 80 } = options;
  ...
  const initAnimation = () => {
    ...
    const boxes = Array.from(node.querySelectorAll<HTMLElement>(".char-box"));
    animate(
      boxes,
      { transform: ["translateY(15px)", "translateY(0px)"] } as any,
      { delay: stagger(delay), duration: duration, easing: [0.19, 1, 0.22, 1] } as any,
    );
    ...
    glitchInterval = setInterval(() => {
      ...
      if (frameCounter > boxes.length * 2 + 6) clearInterval(glitchInterval);
    }, intervalSpeed);
  };
  ...
  const observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting && !node.dataset.initialized) {
